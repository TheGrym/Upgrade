...
export default function App() {
  ...
  const handleStartSession = useCallback((programId, sessionKey) => {
    ...
    if (s) {
      // Appliquer les variantes selon le bloc actuel
      const blockInfo = getCurrentBlock(programId);
      const isBlockB = blockInfo.currentBlock === 1;
      const sessionWithVariants = {
        ...s,
        blockInfo: getBlockDisplayInfo(programId),
        exercises: s.exercises.map(ex => applyVariant(ex, isBlockB)),
      };
